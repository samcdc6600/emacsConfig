...
;; SB ==========================================================================
;; This should be the same as sb-fci-column-width in init.el
;; (setq sb-fci-column-width-minimap 80)
 
 
;; (defun sb-resize-window-to (desired-column-width)
;;   "Tries to resize the current window to it's argument \"desired-column-width\" if it is less than \"desired-column-width\" characters wide."
;;   (when (< (window-width) desired-column-width)
;;     (let ((times-to-call (- desired-column-width (window-width))))
;;       (dotimes (_ times-to-call)
...
(define-minor-mode minimap-mode
  ...
  (if minimap-mode
	...
	(when (and minimap-major-modes
	  ...
	  (minimap-new-minimap)
	  ;; SB ================================================================
	  ;; Try to enlarge non-minimap window if it is less than sb-fci-column-width-minimap columns wide.
	  ;; (sb-resize-window-to sb-fci-column-width-minimap)
	  ;; SB ================================================================
	  )
	;; Create timer.
	(setq minimap-timer-object
